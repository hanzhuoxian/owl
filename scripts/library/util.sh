...
# Example:  util::wait_for_success 120 5 "ctl get nodes|grep localhost"
...
util::ensure-temp-dir() {
  if [[ -z ${TEMP_DIR-} ]]; then
    TEMP_DIR=$(mktemp -d 2>/dev/null || mktemp -d -t owlrnetes.XXXXXX)
...
# Run all known doc generators (today gendocs and genman for owlctl)
...
util::gen-docs() {
  ...
  genowldocs=$(util::find-binary "genowldocs")
...
util::git_upstream_remote_name() {
  git remote -v | grep fetch |\
    grep -E 'github.com[/:]hanzhuoxian/owl' |\
